    ...
    <section class="controls">
      <div class="control-block">
        ...
        <div class="grid">
          ...
          <label>Exponent p<input id="expo" type="number" step="0.1" value="1.3"></label>
          <label>r_mid (m)<input id="rmid" type="number" step="0.001" value="0.060"></label>
          <label>ρ_mid<input id="rhomid" type="number" step="1" value="1080"></label>
          <label>η_mid<input id="etamid" type="number" step="0.0001" value="0.0016"></label>
        </div>
      </div>
      ...
      <div class="control-block">
        ...
        <div class="grid">
          ...
          <label>D50 (nm)<input id="d50" type="number" step="1" value="25"></label>
          <label>g_sigma<input id="gsigma" type="number" step="0.1" value="1.4"></label>
          <label>D50_2 (nm)<input id="d50_2" type="number" step="1" value="12"></label>
          <label>g_sigma2<input id="gsigma2" type="number" step="0.1" value="1.6"></label>
          <label>weight1<input id="w1" type="number" step="0.1" value="0.5"></label>
          <label>weight2<input id="w2" type="number" step="0.1" value="0.5"></label>
          <label>n bins<input id="nbins" type="number" step="1" value="6"></label>
          <label>Disc d1 (nm)<input id="disc_d1" type="number" step="1" value="20"></label>
          <label>Disc w1<input id="disc_w1" type="number" step="0.1" value="1"></label>
          <label>Disc d2 (nm)<input id="disc_d2" type="number" step="1" value="35"></label>
          <label>Disc w2<input id="disc_w2" type="number" step="0.1" value="1"></label>
        </div>
      </div>
...
function render(): void {
  ...
  const palette = ['#6bd0ff', '#ff9f6b', '#9c7bff', '#5de6a9', '#f25f87', '#b6ff6b']
...
function updateTelemetry(effectiveScale: number): void {
  const dtStable = solver.computeStableDt()
  const masses = solver.computeMasses()
  timeEl.textContent = `${(simTime / 3600).toFixed(2)} h`
  dtEl.textContent = Number.isFinite(dtStable) ? `${dtStable.toExponential(2)} s` : '—'
  massEl.textContent = `ΣM=${masses.total.toExponential(3)} | per bin: ${Array.from(
    masses.perBin,
  )
    .map((m) => m.toExponential(2))
    .join(', ')}`
  speedEl.textContent = `${effectiveScale.toFixed(1)}×`
  if (effectiveScale + 1e-6 < timeScale) {
...
function loop(now: number): void {
  const deltaWall = (now - lastTime) / 1000
  lastTime = now
  let effectiveScale = 1
  if (running) {
    const target = timeScale * deltaWall
    const result = solver.advanceBy(target, 400)
    simTime += result.advanced
    effectiveScale = deltaWall > 0 ? result.advanced / deltaWall : 1
  }
  updateTelemetry(effectiveScale)
  render()
